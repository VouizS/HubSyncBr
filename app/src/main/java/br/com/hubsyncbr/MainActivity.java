...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        dualContainer.addView(paneA.container, new LinearLayout.LayoutParams(0, -1, 1));
        LinearLayout.LayoutParams spacerParams = new LinearLayout.LayoutParams(dp(10), -1);
        View spacer = new View(this);
        spacer.setTag("split-spacer");
        dualContainer.addView(spacer, spacerParams);
        dualContainer.addView(paneB.container, new LinearLayout.LayoutParams(0, -1, 1));
    ...
    private LinearLayout createSidebar() {
        ...
        side.addView(navButton("⌂  Home", true));
        side.addView(navButton("▦  Multi Screen", false));
        side.addView(navButton("♡  Favorites", false));
        side.addView(navButton("⚽  Sports", false));
        side.addView(navButton("◎  Browser", false));
        side.addView(navButton("⚙  Settings", false));
    ...
    private LinearLayout createTopBar() {
        ...
        Button swap = chip("↔  Swap Screens", PURPLE);
        swap.setOnClickListener(v -> swapScreens());
        bar.addView(swap, new LinearLayout.LayoutParams(dp(160), dp(42)));
        ...
        more.setOnClickListener(v -> showAboutDialog());
    ...
    private void toggleSplitOrientation() {
        ...
        if (spacer != null) {
            ...
            if (params instanceof LinearLayout.LayoutParams) {
                ...
                lp.width = verticalSplit ? -1 : dp(10);
                lp.height = verticalSplit ? dp(10) : -1;
    ...
    private void exitFocus() {
        ...
        paneA.container.setVisibility(View.VISIBLE);
        paneB.container.setVisibility(View.VISIBLE);
    ...
    private String normalizeUrl(String raw) {
        if (raw == null || raw.trim().isEmpty()) return "https://www.youtube.com";
        String u = raw.trim();
    ...
    private void showLegalNoticeOnce() {
        ...
        if (!prefs.getBoolean("legal_ok", false)) {
            new AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("Entendi", (d, w) -> prefs.edit().putBoolean("legal_ok", true).apply())
                    .show();
        }
    }
    ...
    private void showAboutDialog() {
        new AlertDialog.Builder(this)
                .setTitle("HubSyncBr 0.1 — Dual View")
                .setMessage("MVP com duas janelas WebView, modo foco, troca de telas, atalhos e fullscreen do player quando o site permitir. Compatibilidade depende das regras de cada site/plataforma.")
    ...
    private class StreamPane {
        ...
        StreamPane(Context ctx, String slot, String name, String url, int accent) {
            ...
            close.setOnClickListener(v -> webView.loadUrl("about:blank"));
        ...
        String currentUrl() {
            ...
            if (current == null || current.trim().isEmpty() || current.equals("about:blank")) current = urlBar.getText().toString();
        ...
        void toggleDesktopMode() {
            ...
            if (desktopMode) {
                settings.setUserAgentString("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124 Safari/537.36 HubSyncBr/0.1");
    ...
    protected void onDestroy() {
        super.onDestroy();
        try { paneA.webView.destroy(); } catch (Exception ignored) {}
        try { paneB.webView.destroy(); } catch (Exception ignored) {}
    }
}
